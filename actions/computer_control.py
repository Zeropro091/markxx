...
def _get_os() -> str:
    try:
        s = load_settings()
        return (s.os_system or platform.system() or "windows").lower()
    except Exception:
...
def _screen_find(description: str) -> tuple[int, int] | None:
    ...
    if not api_key:
        _print("[ComputerControl] Warning: No API key for screen_find")
        return None
    ...
    try:
        ...
        _print(f"[ComputerControl] Warning: screen_find failed: {e}")
...
def computer_control(
    ...
    _print(f"[ComputerControl] Run action: {action} with parameters {params}")
    ...
    try:
        ...
        if action == "random_data":
            ...
            _print(f"[ComputerControl] Random {dt} -> {result}")
            return result
        ...
        if action == "user_data":
            ...
            if not value:
                ...
                _print(f"[ComputerControl] Warning: No '{field}' in memory, using random: {value}")
            return value

        return f"Unknown action: '{action}'"

    except Exception as e:
        _print(f"[ComputerControl] Error in action {action}: {e}")
