...
# Reconfigure stdout/stderr encoding to utf-8 if possible
try:
...
def _real_profile_dir(browser: str) -> str:
    ...
    for p in candidates:
        if p.exists():
            _print(f"[Browser] ✅ Real profile found for {browser}: {p}")
            return str(p)

    fallback = home / ".jarvis_profiles" / browser
    fallback.mkdir(parents=True, exist_ok=True)
    _print(f"[Browser] ⚠️  Real profile not found for {browser}, using: {fallback}")
    return str(fallback)
...
def _firefox_profile_dir() -> Optional[str]:
    ...
    if default_path and Path(default_path).exists():
        _print(f"[Browser] Firefox real profile: {default_path}")
        return default_path
    return None
...
def _find_opera_windows() -> Optional[str]:
    ...
    for p in candidates:
        if p.exists():
            _print(f"[Browser] Opera found at: {p}")
            return str(p)
            ...
            for hive in (winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER):
                try:
                    ...
                    if exe and Path(exe).exists():
                        _print(f"[Browser] Opera found via registry: {exe}")
                        return exe
                except Exception:
                    continue
    except Exception:
...
def _resolve_browser(name: str) -> dict | None:
    ...
    if spec.get("special") == "opera_windows":
        ...
        if not exe:
            _print(f"[Browser] ⚠️  Opera executable not found on Windows.")
...
class _BrowserSession:
    """
    A single browser session. Opened with launch_persistent_context.
    """
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                _print(f"[Browser] Firefox real profile failed ({e}), using JARVIS profile")
            ...
            _print(f"[Browser] ✅ Firefox launched")
            return
        ...
        if engine_name == "webkit":
            ...
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            _print(f"[Browser] ✅ Safari launched")
            return
        ...
        try:
            ...
            await asyncio.sleep(0.5) 
            self._page = await self._context.new_page()
            _print(f"[Browser] ✅ Launched [{label}] profile={profile}")
            return
        except Exception as e:
            _print(f"[Browser] ⚠️  Real profile failed for {label}: {e}")

        jarvis_profile = str(Path.home() / ".jarvis_profiles" / self.browser_name)
        Path(jarvis_profile).mkdir(parents=True, exist_ok=True)
        _print(f"[Browser] Retrying with JARVIS profile: {jarvis_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(jarvis_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            _print(f"[Browser] ✅ Launched [{label}] with JARVIS profile")
    ...
    def _get_or_create(self, browser_name: str) -> _BrowserSession:
        with self._lock:
            if browser_name not in self._sessions:
                ...
                _print(f"[Registry] New session: {browser_name}")
