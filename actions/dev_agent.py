...
Code for {file_path}:"""
    ...
    try:
        ...
        code = _strip_fences(response.text or "")

        full_path = project_dir / file_path
        full_path.parent.mkdir(parents=True, exist_ok=True)
        full_path.write_text(code, encoding="utf-8")

        _print(f"[DevAgent] Written: {file_path} ({len(code)} chars)")
        return code
...
def _install_dependencies(dependencies: list[str], project_dir: Path) -> str:
    ...
    for dep in dependencies:
        ...
        if result.returncode != 0:
            ...
            _print(f"[DevAgent] Already installed: {pkg_name}")

    if not to_install:
        return f"All dependencies already installed: {', '.join(dependencies)}"

    _print(f"[DevAgent] Installing: {to_install}")
    try:
        result = subprocess.run(
...
def _open_vscode(project_dir: Path) -> bool:
    ...
    for cmd in vscode_candidates:
        try:
            ...
            time.sleep(1.5)
            _print(f"[DevAgent] VSCode opened: {project_dir}")
            return True
        except Exception:
            continue
    return False
...
def _run_project(run_command: str, project_dir: Path, timeout: int = 30) -> str:
    _print(f"[DevAgent] Running: {run_command}")
    try:
...
def _try_auto_install(error_output: str, project_dir: Path) -> bool:
    """If ModuleNotFoundError exists, try to auto-install it."""
    pattern = re.compile(
    ...
    _print(f"[DevAgent] Auto-installing missing package: {pkg}")
    try:
        result = subprocess.run(
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            fixed = _strip_fences(response.text or "")
            ...
            _print(f"[DevAgent] Fixed: {fix_path}")

        except Exception as e:
            if _is_rate_limit(e):
                raise RateLimitError(str(e))
            _print(f"[DevAgent] Warning: Could not fix {fix_path}: {e}")
