...
def volume_set(value: int):
    ...
    if _OS == "Windows":
        try:
            ...
            _print(f"[Settings] pycaw failed, using keypress fallback: {e}")
            pyautogui.press("volumemute")
            pyautogui.press("volumemute")
    elif _OS == "Darwin":
...
def brightness_up():
    if _OS == "Darwin":
        ...
        try:
            ...
            _print(f"[Settings] Brightness up failed on Windows: {e}")
...
def brightness_down():
    if _OS == "Darwin":
        ...
        try:
            ...
            _print(f"[Settings] Brightness down failed on Windows: {e}")
...
def sleep_display():
    if _OS == "Windows":
        try:
            ...
            _print(f"[Settings] sleep_display failed: {e}")
    elif _OS == "Darwin":
        subprocess.run(["pmset", "displaysleepnow"], capture_output=True)
    else:
...
def dark_mode():
    if _OS == "Darwin":
        ...
        try:
            ...
            _print(f"[Settings] dark_mode registry failed: {e}")
    else:
        try:
            result = subprocess.run(
            ...
            _print(f"[Settings] dark_mode Linux failed: {e}")
...
def toggle_wifi():
    if _OS == "Darwin":
        ...
        try:
            ...
            _print(f"[Settings] toggle_wifi Windows failed: {e}")
    else:
        try:
            ...
            _print(f"[Settings] toggle_wifi Linux failed: {e}")
...
def _detect_action(description: str) -> dict:
    ...
    if not api_key:
        _print("[Settings] Warning: No API key for intent detection")
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        ...
        _print(f"[Settings] Intent detection failed: {e}")
        return {"action": description.lower().replace(" ", "_"), "value": None}
...
def computer_settings(
    ...
    _print(f"[Settings] Action: {action}  Value: {value}  OS: {_OS}")
    if player:
    ...
    try:
        ...
        _print(f"[Settings] Action failed ({action}): {e}")
